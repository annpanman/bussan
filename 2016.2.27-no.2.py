...
for ggg in range(5):
    ...
    for ggh in range(47):
        n[ggg]=a[ggh]
        kakuritu[ggg][yaku(n)-1]+=1

zzzz=0
for ggi in range(3):
    for ggj in range(ggi+1,4):
        for ggt in range(ggj,5):
            zzzz+=1
            for ggk in range(46):
                for ggl in range(ggk+1,47):
                    n=[g[ggi],g[ggj],g[ggt],a[ggk],a[ggl]]
                    kakuritu[zzzz+4][yaku(n)-1]+=1
